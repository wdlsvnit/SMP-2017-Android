...
char* indexes(const char* path)
{
    if (path != NULL) {
        ...
        if (file != NULL) return dir = "/path/to/a/directory/index.php";
        
        else {
            fclose(file);
            file = fopen("index.html", "r");
            
            if (file != NULL) return dir = "/path/to/a/directory/index.html"; 
        } 
        fclose(file);
...
const char* lookup(const char* path)
{
    if(path != NULL)
    {
        if(strcasecmp(path,"CSS") == 0) 
        return "text/css";
         if (strcasecmp(path, "HTML") == 0) 
         return "text/html";
        if (strcasecmp(path, "GIF") == 0)
        return "image/gif";
        if (strcasecmp(path, "ICO") == 0)
        return "image/x-icon";
        if (strcasecmp(path, "JPG") == 0) 
        return "image/jpeg";
        if (strcasecmp(path, "JS") == 0) 
        return "text/javascript";
        if (strcasecmp(path, "PHP") == 0) 
        return "text/x-php";
        if (strcasecmp(path, "PNG") == 0) 
        return "image/png";
    }
    return NULL;
}
